...
router.get('/api/feature-map', authenticateToken, async (req, res) => {
    try {
        if (!req.userId) {
        ...
        if (typeof parsed === 'object' && !Array.isArray(parsed)) {
            ...
            featureArray = Object.values(parsed).map((entry) => ({
...
router.get('/api/github/repos', authenticateToken, async (req, res) => {
    try {
        if (!req.userId) {
            return res.status(401).json({ error: 'Authentication required' });
        }
...
router.post('/api/auth/logout', authenticateToken, (_req, res) => {
    res.status(204).send();
});
