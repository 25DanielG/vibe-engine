...
router.post("/create-feature-map", authenticateToken, async (req, res) => {
    try {
        ...
        const repo = await fetchAllFilesFromRepo({ owner: githubUser, repo: repoName, token: token });
                ...
                mapFuncCalls.forEach((func) => {
                    ...
                    if (funcArg &&
                        ...
                        if (featureGroup[nodeName]) {
                            ...
                            const connectedNames = funcArg.connected_features.items.filter((name) => typeof name === 'string');
...
async function makeFeatureMap(features) {
...
router.post("/generate-feature", authenticateToken, async (req, res) => {
    try {
        ...
        const repo = await fetchAllFilesFromRepo({
            owner: githubUser,
            repo: repoName,
            token: githubToken,
        });
        ...
        if (response.functionCalls && response.functionCalls.length > 0) {
            response.functionCalls.forEach((func) => {
                ...
                if (funcName === "update_file") {
                    writeFileToRepo(githubUser, repoName, 
                    // @ts-ignore
                    funcArgs?.filename, funcArgs?.content, "VibeEngine updated a file in the repository.", "main", githubToken);
                }
                else if (funcName == "add_file") {
                    writeFileToRepo(githubUser, repoName, 
                    // @ts-ignore
                    funcArgs?.filename ?? "", funcArgs?.content ?? "", "VibeEngine added a file to the repository.", "main", githubToken);
                }
            });
